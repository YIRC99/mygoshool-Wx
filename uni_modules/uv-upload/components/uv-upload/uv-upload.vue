      ...
      <template v-if="previewImage">
        <view class="uv-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
          <image v-if="item.isImage || (item.type && item.type === 'image')" :src="item.thumb || item.url"
            :mode="imageMode" class="uv-upload__wrap__preview__image" @tap="onPreviewImage(item,index)" :style="[{
        ...
        <view @tap="chooseFile">
          <slot>
            <view class="uv-upload__button" :hover-class="!disabled ? 'uv-upload__button--hover' : ''"
              hover-stay-time="150" @tap.stop="chooseFile" :class="[disabled && 'uv-upload__button--disabled']" :style="[{
								width: $uv.addUnit(width),
								height: $uv.addUnit(height)
							}]">
    ...
    methods: {
      ...
      onPreviewImage(item, index) {
        ...
        uni.previewImage({
          ...
          urls: this.lists.filter((item) => this.accept === 'image' || image(item.thumb || item.url)).map((item) =>
            item.thumb || item.url),
          current: findIndex,
          fail() {
